...
!										
! ifort -mcmodel=medium -assume byterecl -O2 -o ptrack3_const_ini_den.exe ./UtilLib/compute_zcor.f90 ./UtilLib/schism_geometry.f90 ptrack3_const_ini_den.f90 -I$NETCDF/include -I$NETCDF_FORTRAN/include -L$NETCDF_FORTRAN/lib -L$NETCDF/lib -lnetcdf -lnetcdff
...
!...  	Important variables
        integer, save :: np,ne,ns,nvrt,mnei,mod_part,ibf,istiff,ivcor,kz,nsig
      	real(kind=dbl_kind), save :: h0,rho0,dt,bkt
...
!...    Output handles
        ...
        real(kind=dbl_kind),save, allocatable :: temp1(:,:),salt1(:,:),temp2(:,:),salt2(:,:),solar1(:),solar2(:) !-jx
...
!...  Main program
     ...
     ! character(len=30) :: file63
      ...
      character(len=200) :: fnc_dir
      real(kind=sng_kind) :: floatout,floatout2
      real*8, allocatable :: xpar(:),ypar(:),zpar(:),st_p(:),upar(:),vpar(:),wpar(:),temp_par(:),salt_par(:),solar_par(:) !-jx
     ...
     &csl(4,2),ctl(4,2),csn(4),ctn(4),                                   &
     &dx(10),dy(10),dz(10),val(4,2),vbl(4,2),vcl(4,2),vdl(4,2),van(4),vcn(4),&
     &vdn(4),vwx(4),vwy(4),vs(4),vDIN(4),vTSS(4)
      real, allocatable :: real_ar(:,:)
      integer :: nodel2(3), len_char1, len_char
...
!      if(mod_part==1.and.ibf/=1) stop 'Oil spill must have ibf=1'
     ...
     &temp_par(nparticle),salt_par(nparticle),den_hab(nparticle),C1(nparticle),C2(nparticle),&
     ...
     &solar_par(nparticle),DIN_par(nparticle),TSS_par(nparticle),fI(nparticle),fN(nparticle),&
...
!- read initial C1 (cell density: cell/m3)
!      open(96,file='ini_den.txt',status='old')
!      read(96,*) !comment
!      do i=1,nparticle
!        read(96,*)tmp, C1(i)
!       ! print*, C1(i)
!      enddo
!      print*, 'Finish reading initial density'
!      close(96)
...
!     has different conn table from hgrid.gr3)
      ...
      len_char1=len_trim(fnc_dir)
      file63=fnc_dir(1:len_char1)//'schout_'//ifile_char(1:len_char)//'.nc'
      write(*,*)file63
      ...
      iret=nf90_inq_varid(ncid,'solar_radiation',isolar_id)  !jx
      if(iret/=nf90_NoErr) stop 'solar_radiation not found'  !jx
      ...
      iret=nf90_inq_varid(ncid,'temp',ltemp)  
      if(iret/=nf90_NoErr) stop 'temp not found' 
      iret=nf90_inq_varid(ncid,'salt',lsalt) 
...
!      endif !mod_part
     ...
     &wnx2(np),wny1(np),wny2(np),dldxy(3,2,ne),real_ar(nvrt,np),solar1(np),solar2(np),stat=istat)
      if(istat/=0) stop 'failed to allocate (3)'

     !-read DIN data
      allocate(t_DIN(366),DIN_all(366,np),t_TSS(366),TSS_all(366,np))
      open(98,file='DIN_2021.txt',status='old')
      do i=1,366
         read(98,*)t_DIN(i),DIN_all(i,1:np)
      enddo   
      close(98)

      open(99,file='TSS_2021.txt',status='old')
      do i=1,366
...
!     vgrid
     ...
     &temp1(np,nvrt),salt1(np,nvrt),temp2(np,nvrt),salt2(np,nvrt),&
...
!     Initialize for output before particle moving
      ...
      if(ibf==1) then
       ...
       ! it2=1
        it2=bkt*86400/dt ! control the end time of backward tracking
        irec1=nrec
      endif
...
!        endif !mod_part
      ...
      !-jx
      iret=nf90_get_var(ncid,ltemp,real_ar(1:nvrt,1:np),(/1,1,irec1/),(/nvrt,np,1/))
      if(iret/=nf90_NoErr) stop 'temp2 not read'
      temp2(:,:)=transpose(real_ar(1:nvrt,1:np))
      iret=nf90_get_var(ncid,lsalt,real_ar(1:nvrt,1:np),(/1,1,irec1/),(/nvrt,np,1/))
      if(iret/=nf90_NoErr) stop 'salt2 not read'
...
!...  Deal with junks
      do i=1,np
        if(idry(i)==1) then
          ...
          temp2(i,:)=temp2(i,kbp(i))  !-jx
          salt2(i,:)=salt2(i,kbp(i))  !-jx
...
!...  Store info for first step
      if(it==iths) then
        ...
        temp1=temp2; salt1=salt2; solar1=solar2  !-jx
      endif 
...
!   DIN info
        pt=dt
        if(levpar(1)==-99) pt=(time-st_p(1))*ibf
        dtb=pt/ndeltp
        do idt=1,ndeltp
...
!...    Initialize starting level 
     ...
     &xpar(i),ypar(i),arco,nodel)
            ...
            temp_par(i)=0; salt_par(i)=0; solar_par(i)=0   !-jx
            do j=1,3
              nd=elnode(nodel(j),iel)
              upar(i)=upar(i)+uu2(nd,jlev)*arco(j)
              vpar(i)=vpar(i)+vv2(nd,jlev)*arco(j)
              wpar(i)=wpar(i)+ww2(nd,jlev)*arco(j)
              temp_par(i)=temp_par(i)+temp2(nd,jlev)*arco(j)  !-jx
              salt_par(i)=salt_par(i)+salt2(nd,jlev)*arco(j)
              if(mod_part==1) then
                wndx(i)=wndx(i)+wnx2(nd)*arco(j)
                wndy(i)=wndy(i)+wny2(nd)*arco(j)
                solar_par(i)=solar_par(i)+solar2(nd)*arco(j)  !jx
              endif !mod_part
            enddo !j
...
!          endif
      ...
      !-jx add swimming and settling
            t_swm=time+(idt-1)*dtb 
            swm_ini=DMOD(t_swm,86400.)/3600.
            swm_spd=30./86400
           ! swm_spd=0.
           ! set_spd=5.787e-6 !settling vel=0.5m/d
            set_spd=0.
            if(swm_ini>=6.and.swm_ini<=18) then ! swim upward during
                                                 !6~18h 
              zt=z0+zadv+zdif+dtb*(swm_spd-set_spd)
            else
              zt=z0+zadv+zdif+dtb*(-70./86400-set_spd)!downward at night
            endif
     ...
     &nodel2,arco,zrat,nfl,eta_p,dp_p,ztmp,kbpl,ist(i),inbr(i),rnds,pbeach)
         ...
         trat_DIN=(t_swm-t_DIN(i_rec(idt)))/(t_DIN(i_rec(idt)+1)-t_DIN(i_rec(idt)))         
     !     DIN_1 = DIN_all(i_rec(idt),:); DIN_2 = DIN_all(i_rec(idt)+1,:);
       ! print*,shape(DIN_1)
!	  nnel not dry
!	  Interpolate in time
          do j=1,i34(nnel)
            ...
            if(mod_part==1) then
              ...
              vs(j) =solar1(nd)*(1-trat)+solar2(nd)*trat  ! jx solar radiation
         !     vDIN(j)=DIN_1(nd)*(1-trat_DIN)+DIN_2(nd)*trat_DIN  
              vDIN(j)=DIN_all(i_rec(idt),nd)*(1-trat_DIN)+DIN_all(i_rec(idt)+1,nd)*trat_DIN
              vTSS(j)=TSS_all(i_rec(idt),nd)*(1-trat_DIN)+TSS_all(i_rec(idt)+1,nd)*trat_DIN
            endif !mod_part
            do l=1,2
              lev=jlev+l-2
              ...
              ctl(j,l)=temp1(nd,lev)*(1-trat)+temp2(nd,lev)*trat  !-jx
              csl(j,l)=salt1(nd,lev)*(1-trat)+salt2(nd,lev)*trat  !-jx
              if(mod_part==1) then
...
!	  Interpolate in vertical 
          do j=1,i34(nnel)
            ...
            ctn(j)=ctl(j,2)*(1-zrat)+ctl(j,1)*zrat !-jx
            csn(j)=csl(j,2)*(1-zrat)+csl(j,1)*zrat !-jx
            if(mod_part==1) then
...
!	  Interpolate in horizontal
          ...
          temp_par(i)=0; salt_par(i)=0; solar_par(i)=0;
          DIN_par(i)=0; !-jx
          TSS_par(i)=0;
          do j=1,3 !1st tri for quads
            ...
            temp_par(i)=temp_par(i)+ctn(id)*arco(j)  !-jx
            salt_par(i)=salt_par(i)+csn(id)*arco(j)
            if(mod_part==1) then
              ...
              solar_par(i)=solar_par(i)+vs(id)*arco(j) ! solar radiation
              DIN_par(i)=DIN_par(i)+vDIN(id)*arco(j)
              TSS_par(i)=TSS_par(i)+vTSS(id)*arco(j)
            endif !mod_part
          enddo !j
          dhfy(i)=dhfx(i)
...
!         Compute the vertical gradient of diffusivity
       ...
       ! fN=1.;  !nutrient limitation
        fN(i)=DIN_par(i)/(DIN_par(i)+0.028)
        ...
        !   GP(i)=Gmax(i)*Go_P(i)/(Go_P(i)+Go_H(i))
          GP(i)=Go_P(i)
        else  !night
           G(i)=Go_H(i); ! GP=0.
           GP(i)=0;
...
!        GP(i)=Gmax(i)*Go_P(i)/(Go_P(i)+Go_H(i))
        ...
        !  Gnet(i)=G(i)*(1-C1(i)/cap)-Res(i)-G_agg(i)-G_mor(i)
...
!        write(95,'(i12,2(1x,e22.14),1x,f12.3)')i,xout,yout,real(zpar(i)-eta_p),&
!     &temp_par(i),salt_par(i),den_hab(i),chla(i),G(i),Gnet(i)
     write(95,'(i12,2(1x,e22.14),3(1x,f12.3),20(1x,e22.14))')i,xout,yout,zpar(i)-eta_p,&
     &chla(i)

!      write(95,'(i12,2(1x,e22.14),3(1x,f12.3),20(1x,e22.14))')i,xout,yout,zpar(i)-eta_p,&
!     &temp_par(i),salt_par(i),den_hab(i),chla(i),Gnet(i),fI(i),fN(i),G_agg(i),G_mor(i),Res(i),&
!     &f_kd(i),avg_I(i),fmin_INP(i),G(i),Go_P(i),Go_H(i),Gmax(i),GP(i),fT(i),fS(i),TSS_par(i),DIN_par(i)
...
!     &real(temp2(elnode(1:i34(ie4),ie4),levpar(i))),real(salt2(elnode(1:i34(ie4),ie4),levpar(i)))
        endif !levpar
...
!            arco(3), nodel(3) : area coord. and 3 local node indices (valid only if inside)
      ...
      do m=1,i34-2 !# of triangles
        ...
        ae=abs(aa-ar(m))/ar(m)
         if(ae<=ae_min) then
        ! ae=ae_min !-jx 06.19.2021 this is a bug, Joseph fix it in
        !github
          ae_min = ae
          nodel(1:3)=list(1:3)
